...
async fn roles_click(ctx: &serenity::Context, data: &Data, mci: &serenity::ComponentInteraction, id_vec: &mut VecDeque<&str>) -> Result<(), Error> {
	...
	match id_vec.pop_front().unwrap() {
		...
		"pick" => {
			...
			let mci_message = mci.get_response(ctx).await?;
			let sec_mci = match mci_message.await_component_interaction(ctx)
			...
			let selected_roles: Vec<serenity::RoleId> = match sec_mci.data.kind.clone() {
