...
async fn on_join<'a>(
	...
	info!("Joining guild \"{}\" (GuildId {}).", guild.name, guild.id);
	...
	match event {
		...
		serenity::FullEvent::GuildCreate { guild, is_new } => {
			...
			if is_new.is_some_and(|x| x) {
				...
				match actually_new {
					// New
					Ok(n) if n == true => on_join(ctx, framework, guild, &data.db).await?,
					// Error
